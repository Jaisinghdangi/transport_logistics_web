...
$cgst=0;
$sgst=0;
$mainprint = $_GET['invoice_print'];
if($mainprint == ''){
  $starting =1;
  $printpagenew = $printpage;
}else{
  $starting =$mainprint;
  $printpagenew = $mainprint;
}
// print_r( $multiinvocetable);
for($i=$starting; $i <= $printpagenew; $i++){  ?>
    ...
    <div class="row pl-5 pr-0">
      ...
      <div class="col-sm-5 text-right pl-0">
        <div class="side-detail">
          <p>INVOICE NO. <span class="invoice-no"><?//= getVoucherNumber('Invoice') ?> <?= $multiinvocetable[$i-1]->invoice_number; ?></span></p>
...
$total_payable =0;
            ...
            // $total_payable += $split_bill[$i-1]->booking_amt;
        ...
        <?php
        $total_payable += ((($split_bill) ? $split_bill[$i-1]->booking_amt : 0) + (($split_bill) ? $split_bill[$i-1]->rto_fine : 0) + (($split_bill) ? $split_bill[$i-1]->statical_charge : 0) + (($split_bill) ? $split_bill[$i-1]->detention_charge_loading 
        :0) + (($split_bill) ? $split_bill[$i-1]->detention_charge_unloading : 0));
        ...
        $gst_data=multiple_calculate_gst($total_payable,$MultiBookingVehical[$i-1]->consignor_id,$MultiBookingVehical[$i-1]->consignee_id,$booking);
          ...
          <?php if($i == $printpagenew){  
            ...
            <div class="below-table below-right d-none">
            <div class="row">
              <div class="w-90px">
                <p>TOTAL PAYABLE</p>
              </div>
