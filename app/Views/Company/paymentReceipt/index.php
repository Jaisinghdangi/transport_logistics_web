...
if (isset($invoices) && count($invoices) > 0) {
    ...
    foreach ($invoices as $invoice) {
        // Determine the customer name and ID based on the booking type
        if ($invoice->booking_type == 'Paid') {
            $customer_name = get_title('consignors', ['id' => $invoice->billing_customer_id], 'name');
            $customer_id = get_title('consignors', ['id' => $invoice->billing_customer_id], 'id');
            $customer_type = 'Consignor';

        } elseif ($invoice->booking_type == 'To Pay') {
            $customer_name = get_title('consignees', ['id' => $invoice->billing_customer_id], 'name');
            $customer_id = get_title('consignees', ['id' => $invoice->billing_customer_id], 'id');
        ...
        if (!empty($arrinvoice_number)) {
            foreach ($arrinvoice_number as $key1 => $invoice_number) {
                if ($invoice->booking_type == 'Paid') {
                    $consignor_id = get_title_full('multiple_booking_vehical', ['booking_id' => $invoice->booking_id], 'consignor_id')[$key1]->consignor_id;
                    $customer_name = getConsinorName($consignor_id);
                    $customer_id = $consignor_id;
                    $customer_type = 'Consignor';

                } elseif ($invoice->booking_type == 'To Pay') {
                    $consignee_id = get_title_full('multiple_booking_vehical', ['booking_id' => $invoice->booking_id], 'consignee_id')[$key1]->consignee_id;
                    $customer_name = getConsineeName($consignee_id);
...
?>
                                    ...
                                    <div class="col-md-4 col-sm-4 col-lg-4 col-xl-4 col-xxl-4">
                                        <div class="form-group">
                                            <label for="invoices">Select Invoices <span class="text-danger">*</span></label>
                                            <select name="invoices[]" id="invoices" class="select2 form-control form-control-sm" placeholder="Select Invoices" title="Select Invoices" required="" multiple>
                                                <option selected="" disabled="" value="">Select Invoices</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-md-4 col-sm-4 col-lg-4 col-xl-4 col-xxl-4">
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="form-group text-center">
                                            <?php //if ($this->session->userdata('roles') == 3) { if (IsLicensed()) { ?>
...
</button>                              <input type="hidden" value="" name="party_txt_name" id="party_txt_name">
                                </div>
                            </div>
                            <div class="card-body">
...
<script type="text/javascript">
  ...
  var tableRows = table.find('tbody tr').length;
    ...
    var tableHtml = $('#allPayments')[0].outerHTML;
     ...
     completeHtml = `
                    <div id="pdf-content">
                        ...
                        <div class="header">
                            <h2>Invoice Payment Statement</h2>
                ...
                `;
        //         $.ajax({
        //             url: '<?= base_url('company/payment-statement-pdf') ?>',
        //             method: 'POST',
        ...
        //                  link.download = 'Invoice_Payment.pdf';
        //                  link.click();
        //             },
        ...
        $('#loadingGif').show();
                ...
                // Convert the HTML to canvas using html2canvas and then to PDF using jsPDF
        ...
        // });
          ...
          $('#invoices').change(function(){
                ...
                // console.log("Total Invoice Amount: " + totalInvoiceAmt);
            });
    ...
    var url = '<?= base_url('company/get-invoice-list') ?>';
    // console.log('URL:', url);
    ...
    $.ajax({
        ...
        success: function(data) {
            // console.log('Response:', data.blanceAmount); // Inspect the response
            if (data) {
 ...
 $("#dataForm").submit(function(e) {
    ...
    $.ajax({
        url: '<?= base_url('company/save-invoice-list') ?>', 
        method: 'post',
        data: formData,
        processData: false,
        contentType: false,
